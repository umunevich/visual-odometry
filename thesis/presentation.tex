...
\begin{frame}{Мета та завдання}
  ...
  \begin{enumerate}
    ...
    \item перевірити на даних EuRoC MAV.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Наочна демонстрація}
  \small
  \textbf{Підготовка} (до захисту):
  \begin{itemize}
    \item \texttt{docker compose up --build};
    \item \url{http://localhost:4200}.
  \end{itemize}

  \textbf{На захисті} (≈1{,}5~хв):
  \begin{enumerate}
    \item «З файлу» → відео EuRoC → профіль Dataset → \textbf{Start VO!}
    \item Показати 3D-траєкторію, \texttt{tracking}, \texttt{confidence}
    \item (Опційно) «Потік» — кілька секунд з вебкамери
  \end{enumerate}

  \centering
  \maybeincludegraphics[0.42\textwidth]{\UiScreenshot}
\end{frame}
...
\begin{frame}{Обмеження та перспективи}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}
      ...
      \item умовний масштаб траєкторії;
    ...
    \textbf{Перспективи}
    \begin{itemize}
      \item експорт траєкторії, метрики ATE/RPE (evo);
...
\begin{frame}{Висновки}
  \begin{itemize}
    ...
    \item На EuRoC: похибка репроєкції \textbf{1{,}21~px}; підтверджено коректність VO-конвеєра.
