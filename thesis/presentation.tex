...
\begin{frame}{Постановка задачі}
  \textbf{Проблема:} без GNSS БПЛА потребує оцінки траєкторії за бортовим відео однієї камери.

  \medskip
  \textbf{Задача:} побудувати прототип, який з відео польоту MAV формує 3D-шлях і дозволяє порівняти його з еталоном.

  \medskip
  \textbf{Критерії успіху:}
  \begin{itemize}
    \item стабільне відстеження на записі EuRoC (\texttt{V1\_01\_easy});
    \item ATE після Sim(3) порядку метра;
    \item візуалізація та експорт траєкторії для аналізу польоту.
  \end{itemize}
\end{frame}

% --- Слайд 4 ---
\begin{frame}{Мета та завдання}
  \textbf{Мета:} розробити веб-систему монокулярної VO для попередньої оцінки траєкторії БПЛА з інтерактивною 3D-візуалізацією.
...
\begin{frame}{Архітектура системи}
  \centering
  \maybeincludegraphics[0.65\textwidth]{fig-03-architecture.png}

  \medskip
  \begin{itemize}
    \item \textbf{Сервер} - VO та калібрування бортової камери (імітація бортового модуля).
    \item \textbf{Клієнт} - прийом відео польоту MAV, 3D-графік для оператора.
    \item \textbf{WebSocket} - кадри (JPEG/Base64) і координати в реальному часі.
  \end{itemize}
\end{frame}
...
\begin{frame}{Конвеєр візуальної одометрії}
  \centering
  \maybeincludegraphics[0.68\textwidth]{fig-04-vo-pipeline.png}
...
\begin{frame}{Технологічний стек}
  ...
  \medskip
  Режими: \textbf{«Потік»} (імітація онлайн) та \textbf{«З файлу»} (запис польоту EuRoC MAV).
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{table}
      ...
      \begin{tabular}{|l|r|}
        ...
        \hline
        ATE (СКП) & \textbf{1,68~м} \\
        \hline
        $s$ (\text{Sim(3)}) & 0,039 \\
        \hline
        Довжина еталону & 58,6~м \\
        \hline
        Шлях VO (масштаб.) & \textbf{26,2~м} \\
        \hline
      \end{tabular}
    \end{table}

    \vspace{0.5em}
    Для БПЛА: локальна точність $\approx$1,68~м на шляху $\sim$59~м; довжина VO ~45\,\% еталону (обмеження монокуляра).

    \column{0.52\textwidth}
    \centering
    \maybeincludegraphics[0.98\columnwidth]{fig-07-trajectory.png}
  \end{columns}
\end{frame}
...
\begin{frame}{Висновки}
  \begin{itemize}
    \item Сформульовано задачу \textbf{попередньої оцінки траєкторії БПЛА} за бортовим відео.
    \item Реалізовано веб-прототип: VO, калібрування, порівняння з еталоном EuRoC MAV.
    \item На польоті \texttt{V1\_01\_easy}: ATE \textbf{$\approx$1,68~м}, 99,9\,\% успішних кадрів - критерії з постановки виконано.
    \item Обмеження: метричний масштаб у польоті без IMU; довжина шляху $\approx$41\,\% еталону.
    \item Придатно для навчання та попередньої перевірки бортової локалізації MAV.
  \end{itemize}
\end{frame}
